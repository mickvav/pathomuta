...
my $tree1 =  Algorithm::Cluster::treecluster(%param);
$param{transpose}=1;
my $tree2 =  Algorithm::Cluster::treecluster(%param);
my $record1=  Algorithm::Cluster::Record->new();
my $record2=  Algorithm::Cluster::Record->new();

$record1->save(jobname => "tree1", geneclusters => $tree1, expclusters => $tree2);
#my $rehash;
#my $i=0;
#foreach my $r (@$res) {
#  $rehash->{$r}->{$k[$i]}=$i;
